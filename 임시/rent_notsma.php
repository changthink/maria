...
<?php
   ...
   $sql ="SELECT 광역시도, 시자치구, 법정동, 아파트, 전용, 보증금, 월세, 층, 기준월 FROM rent_notsma
          where 광역시도 = '".$광역시도."' AND 시자치구 like '%".$시자치구."%' and 전용 >= '".$전용1."' and 전용 <= '".$전용2."'
          and 기준월 >= '".$from."' and 기준월 <= '".$to."' and 월세 >= '".$월세1."' and 월세 <= '".$월세2."'
          order by 기준월 desc, 보증금 desc LIMIT 50";
   $rst = mysqli_query($con, $sql);   
   
   echo "<h1> 아파트전세 실거래조회결과 </h1>";
   echo "<table border=1";
   echo "<TR>";   
   echo "<TH>광역시도</TH><TH>시자치구</TH><TH>법정동</TH><TH>아파트</TH><TH>전용</TH><TH>보증금</TH><TH>월세</TH><TH>층</TH><TH>기준월</TH>";
   echo "</TR>";
   ...
   while($row = mysqli_fetch_array($rst)) {
	  ...
	  echo "<TD width=150><font size=2.5><p align='center'>", $row['법정동'], "</p></TD>";
     ...
     echo "<TD width=100><font size=2.5><p align='center'>", $row['층'], "</p></TD>";
	  echo "<TD width=100><font size=2.5><p align='center'>", $row['기준월'], "</p></TD>";	  
	  echo "</TR>";	  
   }   
   mysqli_close($con);
   echo "</TABLE>"; 
   echo "<br> <font size=2.5><a href='rp_main4.html'> <-홈으로</a></p>";
?>