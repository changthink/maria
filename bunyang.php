...
<?php
   ...
   $sql ="SELECT 광역시도, 시자치구, 법정동, 아파트, 전용, 층, 거래금액, 기준월 FROM bunyang 
          where 광역시도 = '".$광역시도."' AND 시자치구 like '%".$시자치구."%' and 전용 >= '".$전용1."' and 전용 <= '".$전용2."'         
          and 기준월 >= '".$from."' and 기준월 <= '".$to."'
          order by 기준월 desc LIMIT 100";
   $rst = mysqli_query($con, $sql);   
   
   echo "<h3> 분양권 실거래조회결과 </h3>";   
   echo "<table border=1";
   echo "<TR>";
   ...
   mysqli_close($con);
   echo "</TABLE>"; 
   echo "<br> <font size=2.5><a href='./rp_bunyang.html'> <-홈으로</a></p>";
?>
