...
//    $to = $_POST['조회월2'];
   ...
   $sql ="SELECT 광역시도, 시자치구, 법정동, 아파트, 전용,
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 12 MONTH), 보증금, 0 ) ) AS m12, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 11 MONTH), 보증금, 0 ) ) AS m11, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 10 MONTH), 보증금, 0 ) ) AS m10, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 9 MONTH), 보증금, 0 ) ) AS m9, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 8 MONTH), 보증금, 0 ) ) AS m8, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 7 MONTH), 보증금, 0 ) ) AS m7, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 6 MONTH), 보증금, 0 ) ) AS m6, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 5 MONTH), 보증금, 0 ) ) AS m5, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 4 MONTH), 보증금, 0 ) ) AS m4,
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 3 MONTH), 보증금, 0 ) ) AS m3, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 2 MONTH), 보증금, 0 ) ) AS m2, 
          max( IF( 기준월 = LAST_DAY(NOW() - INTERVAL 1 MONTH), 보증금, 0 ) ) AS m1, 
          max( IF( 기준월 = LAST_DAY(NOW()), 보증금, 0 ) ) AS m0
        FROM rent_notsma
        WHERE 기준월 > LAST_DAY(NOW() - INTERVAL 12 MONTH) AND 광역시도 = '".$광역시도."' and 시자치구 LIKE '%".$시자치구."%' 
        and 전용 >= '".$전용1."' and 전용 <= '".$전용2."' and 월세 = 0
        GROUP BY 광역시도, 시자치구, 법정동, 아파트, 전용 LIMIT 50";

   $rst = mysqli_query($con, $sql);   
   
   echo "<h1> 아파트전세 실거래조회결과(1년) </h1>";  
   echo "<table border=1";
   echo "<TR>";   
   echo "<TH>광역시도</TH><TH>시자치구</TH><TH>법정동</TH><TH>아파트</TH><TH>전용</TH>
