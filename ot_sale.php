...
<?php
   ...
   $sql ="SELECT 광역시도, 시자치구, 법정동, 단지명, 전용, 층, 거래금액, 기준월 FROM ot_sale
          where 광역시도 = '".$광역시도."' AND 시자치구 like '%".$시자치구."%' and 전용 >= '".$전용1."' and 전용 <= '".$전용2."'         
          and 기준월 >= '".$from."' and 기준월 <= '".$to."'
          order by 기준월 desc, 거래금액 desc LIMIT 100";
   $rst = mysqli_query($con, $sql);   
   
   echo "<h3> 오피스텔 매매 실거래조회결과 </h3>";
   echo "<table border=1";
   echo "<TR>";
   echo "<TH>광역시도</TH><TH>시자치구</TH><TH>법정동</TH><TH>단지명</TH><TH>전용</TH><TH>층</TH><TH>거래금액</TH><TH>기준월</TH>";
   echo "</TR>";
   ...
   mysqli_close($con);
   echo "</TABLE>"; 
   echo "<br> <font size=2.5><a href='rp_main.html'> <-조회화면으로</a></p>";
?>